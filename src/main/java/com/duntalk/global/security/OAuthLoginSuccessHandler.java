...
public class OAuthLoginSuccessHandler
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        SocialProvider provider = SocialProvider.from(oauth.getAuthorizedClientRegistrationId());
        ...
        Optional<Member> member = memberService.findBySocialAccount(provider, providerId);

        if (member.isPresent()) {
            memberService.login(member.get(), request, response);

            response.sendRedirect("/");
            return;
        }

        request.getSession().setAttribute(PendingSignup.SESSION_KEY, new PendingSignup(provider, providerId, email));

        response.sendRedirect("/signup?provider=" + provider.name());
    }
}
